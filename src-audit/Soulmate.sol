...
contract Soulmate is ERC721 {
    ...
    function mintSoulmateToken() public returns (uint256) {
        ...
        // @todo @audit too many storage reads for nextID
        ...
        if (soulmate1 == address(0)) {
            ...
            // @todo @audit soulmate2 is going to be zero address, use msg.sender instead
            emit SoulmateAreReunited(soulmate1, soulmate2, nextID);

            // @question-a why does soulmate1 not get the token?
            // @answer because the token is minted only for one of the soulmate
