...
contract Staking {
    ...
    function deposit(uint256 amount) public {
        ...
        // @todo @audit we should claim rewards to update timestamp for rewards
    ...
    function claimRewards() public {
        // @todo @audit add validation to check if userStakes > 0

        uint256 soulmateId = soulmateContract.ownerToId(msg.sender);
        // first claim
        if (lastClaim[msg.sender] == 0) {
            // @todo @audit it should be block.timestamp, else users can claim rewards for duration
            // they didn't stake but only owned the soulmate NFT
        ...
        // @todo @audit add validation that the contract has enough LoveToken to transfer
